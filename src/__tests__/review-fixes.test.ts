...
describe('review: la partenza notturna', () => {
  ...
  test('un temporale dopo la mezzanotte non sparisce dal verdetto', () => {
    ...
    const r = buildRouteWeather({
      ...
      serie: [{ time, cape, weather_code, wind_gusts_10m, precipitation_probability, temperature_2m: [], precipitation: [] }],
    });
...
describe('review 2: una serie per punto, o niente', () => {
  const serie = (cape: number) => {
    ...
    return { time, cape: c, weather_code: wc, wind_gusts_10m: g, precipitation_probability: pp, temperature_2m: [], precipitation: [] };
  };
